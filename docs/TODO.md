...
- [ ] `app/page.tsx` 업데이트
  - [ ] 기본 레이아웃 구조 (헤더, 메인 영역)
  - [ ] 검색창, 필터, 목록, 지도 영역 구분
  - [ ] 반응형 레이아웃 (데스크톱: 분할, 모바일: 탭/스택)

### 2.2 관광지 목록 컴포넌트

- [ ] `components/tour-card.tsx` 파일 생성
  - [ ] 썸네일 이미지 표시 (없으면 기본 이미지)
  - [ ] 관광지명 표시
  - [ ] 주소 표시
  - [ ] 관광 타입 뱃지 표시
  - [ ] 간단한 개요 (1-2줄)
  - [ ] 클릭 시 상세페이지 이동 (`/places/[contentId]`)
  - [ ] 호버 효과 및 스타일링
- [ ] `components/tour-list.tsx` 파일 생성
  - [ ] 관광지 목록을 그리드로 표시
  - [ ] 로딩 상태 (Skeleton UI)
  - [ ] 빈 상태 처리 (결과 없음)
  - [ ] 페이지네이션 또는 무한 스크롤
...
- [ ] `components/tour-filters.tsx` 파일 생성
  - [ ] 지역 필터 (시/도 선택)
    - [ ] `areaCode2` API 연동하여 지역 목록 조회
    - [ ] 드롭다운 또는 체크박스 UI
    - [ ] "전체" 옵션
  - [ ] 관광 타입 필터
    - [ ] ContentTypeId 선택 (12, 14, 15, 25, 28, 32, 38, 39)
    - [ ] 드롭다운 또는 체크박스 UI
    - [ ] "전체" 옵션
  - [ ] 필터 상태 관리 (URL 쿼리 또는 상태)
  - [ ] 필터 초기화 기능
- [ ] 필터 동작 연결
...
- [ ] `components/tour-search.tsx` 파일 생성
  - [ ] 검색창 UI (헤더 또는 메인 영역)
  - [ ] 검색 아이콘 표시
  ...
  - [ ] 검색어 초기화 기능
- [ ] 검색 API 연동
  - [ ] `searchKeyword2` API 호출
  - [ ] 검색 결과를 `tour-list`에 표시
  - [ ] 검색 결과 개수 표시
  - [ ] 결과 없음 안내 메시지
- [ ] 검색 + 필터 조합
  - [ ] 키워드 + 지역 필터 동시 적용
  - [ ] 키워드 + 관광 타입 필터 동시 적용
  - [ ] 모든 필터 동시 적용 가능하도록 구현

### 2.5 네이버 지도 연동

- [ ] `components/naver-map.tsx` 파일 생성
  - [ ] 네이버 지도 기본 표시 (Naver Maps API v3 NCP)
  - [ ] `ncpKeyId` 파라미터 사용 (환경변수 `NEXT_PUBLIC_NAVER_MAP_CLIENT_ID`)
  - [ ] 초기 중심 좌표 설정 (선택된 지역 또는 기본값)
  - [ ] 줌 레벨 자동 조정
- [ ] 지도 마커 기능
  - [ ] 관광지 목록을 마커로 표시
  - [ ] KATEC 좌표계 변환 (mapx, mapy를 10000000으로 나누기)
  - [ ] 마커 클릭 시 인포윈도우 표시
    - [ ] 관광지명
    - [ ] 간단한 설명
    - [ ] "상세보기" 버튼 (상세페이지 이동)
...
- [ ] 반응형 레이아웃
  ...
  - [ ] 모바일: 탭 형태로 리스트/지도 전환 또는 스택 레이아웃
...
- [ ] 정렬 기능
  - [ ] 정렬 옵션 UI 추가 (최신순, 이름순)
  - [ ] `modifiedtime` 기준 최신순 정렬
  - [ ] 관광지명 기준 가나다순 정렬
- [ ] 페이지네이션
  - [ ] 페이지당 10-20개 항목 제한
  - [ ] 페이지 번호 선택 또는 무한 스크롤 구현
  - [ ] API 파라미터 `numOfRows`, `pageNo` 연동
- [ ] 로딩 상태 개선
  - [ ] Skeleton UI로 교체 (카드 스켈레톤)
  - [ ] 스피너 대신 자연스러운 로딩 경험
...
- [ ] `app/places/[contentId]/page.tsx` 파일 생성
  - [ ] 동적 라우팅 파라미터 처리 (`contentId`)
  - [ ] 기본 레이아웃 구조 (뒤로가기 버튼, 섹션 구분)
  - [ ] 로딩 상태 처리
  - [ ] 에러 처리 (404, API 에러)
  - [ ] 라우팅 테스트 (홈에서 카드 클릭 시 이동)

### 3.2 기본 정보 섹션

- [ ] `components/tour-detail/detail-info.tsx` 파일 생성
  - [ ] `detailCommon2` API 연동
  - [ ] 관광지명 표시 (대제목)
  - [ ] 대표 이미지 표시 (크게)
  - [ ] 주소 표시 및 복사 기능
  - [ ] 전화번호 표시 및 클릭 시 전화 연결 (`tel:` 링크)
  - [ ] 홈페이지 표시 (링크)
  - [ ] 개요 표시 (긴 설명문)
  - [ ] 관광 타입 및 카테고리 뱃지 표시
  - [ ] 정보 없는 항목 숨김 처리
...
- [ ] `components/tour-detail/detail-map.tsx` 파일 생성
  - [ ] 해당 관광지 위치만 지도에 표시 (마커 1개)
  - [ ] KATEC 좌표계 변환 (mapx, mapy)
  - [ ] 지도 중앙을 관광지 위치로 설정
  - [ ] "길찾기" 버튼 추가
    - [ ] 네이버 지도 앱/웹 연동 (`https://map.naver.com/...`)
    - [ ] 좌표 정보 활용
  - [ ] 좌표 정보 표시 (선택 사항)

### 3.4 공유 기능

- [ ] `components/tour-detail/share-button.tsx` 파일 생성
...
- [ ] Open Graph 메타태그 동적 생성
  - [ ] `app/places/[contentId]/page.tsx`에서 `generateMetadata` 함수 구현
  - [ ] `og:title` (관광지명)
...
- [ ] `components/tour-detail/detail-intro.tsx` 파일 생성
  - [ ] `detailIntro2` API 연동
  - [ ] 운영시간/개장시간 표시
  - [ ] 휴무일 표시
  - [ ] 이용요금 표시
  ...
  - [ ] 타입별 필드 차이 처리 (contentTypeId에 따라)
- [ ] `components/tour-detail/detail-gallery.tsx` 파일 생성
  - [ ] `detailImage2` API 연동
  - [ ] 대표 이미지 + 서브 이미지들 표시
  - [ ] 이미지 클릭 시 전체화면 모달
